    ...
    <div class="container">
        <div class="row timeline">
            ...
            <div class="col-md-6 feed">
                ...
                {!! Form::close() !!}
{{--@dd(Auth::user()->getTimeline())--}}
                @if (Auth::user()->getTimeline()->count())
                    <div class="posts">
                        @foreach (Auth::user()->getTimeline() as $item)
                            @if ($item instanceof App\Post)
                                @include('layouts.posts', ['post' => $item])
                            @elseif ($item instanceof App\Share)
                                @php $sharedByFullName = $item->sharedBy->getFullName(); @endphp
                                @php $sharedId = $item->sharedBy->id; @endphp
                                @include('layouts.share', ['sharedPost' => $item, 'sharedByFullName' => $sharedByFullName, 'sharedId' => $sharedId])
                            @endif
                        @endforeach
                    </div>
                @endif
